...
const closestToZero = (numbers: Array<number>) : number => {
    ...
    const closestToZeroValue = numbers.reduce((a: number, b: number) => Math.abs(a) < Math.abs(b) ? a : b )
    
    // If closestToZeroValue is negative, we check if same positive value exist in array 
    if (closestToZeroValue < 0) {
        const positiveValue = numbers.find((n: number) => Math.abs(closestToZeroValue) === n);
